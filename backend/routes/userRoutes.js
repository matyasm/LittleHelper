...
// Deep debug login route
router.post('/debug-login', async (req, res) => {
  try {
    const { email, password } = req.body;
    
    console.log('====== DEEP DEBUG LOGIN ======');
    console.log(`Attempt for email: ${email}`);
    ...
    // 6. Generate a JWT if any method succeeds
    let token = null;
    if (bcryptMatch || directMatch) {
      token = jwt.sign(
        { id: user._id },
        process.env.JWT_SECRET || 'developmentfallbacksecret',
        { expiresIn: '1h' }
      );
      console.log('Token generated successfully');
    } else {
      console.log('Authentication failed - no token generated');
    }
    
    // 7. Send detailed response
    res.json({
      debug: {
        userFound: true,
        methods: {
          directComparison: directMatch,
          bcryptComparison: bcryptMatch
        },
        passwordDetails: {
          providedPassword: password,
          isHashed,
          hashPrefix: isHashed ? user.password.substring(0, 7) + '...' : 'N/A'
        }
      },
      authenticated: !!(bcryptMatch || directMatch),
      token,
      user: token ? {
        _id: user._id,
        email: user.email,
